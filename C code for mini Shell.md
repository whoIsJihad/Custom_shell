# Reference: full mini-shell code (read for concepts, don’t copy)

This file contains a **complete shell program** copied from elsewhere.
It’s useful to *study* patterns you will need later (especially job control), but it’s not meant to be pasted into your own project.

Important:
- The code includes a license header. Treat it as **third‑party code**.
- Prefer re-deriving the logic yourself from manpages + your own notes.

What to look for when reading (process-management concepts):
- process groups: `setpgid`, `getpgrp`
- controlling terminal: `tcsetpgrp`, `tcgetpgrp`
- reaping: `waitpid(..., WNOHANG)` inside a `SIGCHLD` handler
- foreground vs background: when the parent waits vs doesn’t
- redirection: `open` + `dup2` (later in the file)

If you want a gentler path, start with:
- [index.md](index.md)
